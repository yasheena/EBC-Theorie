...
Several open problems of standard cosmology are addressed within this framework: the nature of dark energy (reinterpreted as a geometric space property), the Hubble tension (attributed to spatial inhomogeneities accumulated over cycles), CMB anomalies, the lithium-7 problem (selective nuclear photodisintegration at the bounce), and baryogenesis (distributed across one or more full cycles). We estimate the current epoch to lie in the 3rd or 4th cycle, approximately 5–15% into the current expansion phase, implying a cycle duration of $T \approx 190$–320 Gyr (from numerical integration with physical matter and radiation content; Section 3.2).
...
The EBC-predicted age follows from integrating EBC1 directly:

$$H_0 \cdot t_0^{(\mathrm{EBC})} = \int_0^1 \frac{dx}{x\,\sqrt{\Omega_m/x^3 + \Omega_r/x^4 + \Omega_{DE}(1/x - x/x_*^2)}} \approx 0.893 \quad(\Rightarrow\; t_0 \approx 12.95\,\mathrm{Gyr})$$

compared to the $\Lambda$CDM value $H_0 \cdot t_0 = 0.951$ ($t_0 \approx 13.8\,\mathrm{Gyr}$), a deficit of approximately **6%** (0.84 Gyr). The origin is structural: $\rho_\mathrm{EBC} \propto a^{-1}$ implies higher dark-energy density at $z > 0$ than $\Lambda$CDM, accelerating the early expansion. An important caveat applies: the reference value $t_0 = 13.8\,\mathrm{Gyr}$ is a $\Lambda$CDM-model-dependent inference from CMB and BAO data, computed using $H_{\Lambda\mathrm{CDM}}(z)$. A self-consistent EBC fit to the same data — using $H_\mathrm{EBC}(z)$ — would yield different values of $H_0$ and $\Omega_m$ and consequently a different inferred $t_0$. The 6% figure is therefore an upper bound on the true model tension, pending such an analysis (identified as a development priority in Section 8). Crucially, the analytic prediction $w_0 = -2/3$ (Section 2.5) is independent of the numerical integration and is unaffected by these corrections.
...
3a. **$H_0 \cdot t_0$ consistency and self-consistent parameter fit**: The EBC Friedmann equation predicts $H_0 \cdot t_0 \approx 0.893$ ($t_0 \approx 12.95\,\mathrm{Gyr}$), approximately 6% below the $\Lambda$CDM-inferred value of $t_0 \approx 13.8\,\mathrm{Gyr}$. This deficit arises structurally from $\rho_\mathrm{EBC} \propto a^{-1}$, which implies higher dark-energy density at $z > 0$ than $\Lambda$CDM. However, the reference value $t_0 = 13.8\,\mathrm{Gyr}$ is itself a $\Lambda$CDM-model-dependent inference: it uses $H_{\Lambda\mathrm{CDM}}(z)$ to interpret CMB and BAO data. A self-consistent EBC analysis must fit $H_\mathrm{EBC}(z)$ directly to BAO distance measurements and CMB angular scales. Since $\rho_\mathrm{EBC}$ at $z \gtrsim 1100$ is negligible compared to radiation and matter, the sound horizon $r_s$ is unchanged; the BAO angular scales $\theta_s(z) = r_s/D_{A,\mathrm{EBC}}(z)$ will differ from $\Lambda$CDM predictions, yielding a corrected effective $H_0$ and consequently a revised $t_0$. This analysis is a development priority for a future revision.
