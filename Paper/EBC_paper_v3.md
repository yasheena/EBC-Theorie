...
Several open problems of standard cosmology are addressed within this framework: the nature of dark energy (reinterpreted as a geometric space property), CMB anomalies, the lithium-7 problem (selective nuclear photodisintegration at the bounce), and baryogenesis (distributed across one or more full cycles). The Hubble tension is not resolved at the background level; EBC shares the "late-time-only" limitation of dynamical dark-energy models with $w_0 > -1$ that leave early-universe physics unchanged (Section 4.2, Section 8 Item 3c). A qualitative mechanism via perturbations accumulated across cycles is outlined but not developed quantitatively. We estimate the current epoch to lie in the 3rd or 4th cycle, approximately 4–7% into the current expansion phase, implying a cycle duration of $T \approx 190$–320 Gyr (from numerical integration with physical matter and radiation content; Section 3.2).
...
The system (EBC1)–(EBC2) has been integrated with physical parameters ($\Omega_m = 0.315$, $\Omega_r = 9.2 \times 10^{-5}$, $1/H_0 = 14.51\,\mathrm{Gyr}$; $\Omega_{DE}$ from the flatness condition at $x = 1$) for the half-cycle from the bounce minimum ($x \approx 0$) to the expansion maximum ($x = x_*$):
...
*$T_\mathrm{free}$: EBC without matter/radiation, same $\Omega_{DE}$. $T_\mathrm{mat}$: physical EBC with $\Omega_m$, $\Omega_r$. Ratio: $T_\mathrm{mat}/T_\mathrm{free}$. $f = t_0/T$ uses the observed $t_0 = 13.8\,\mathrm{Gyr}$.*

The physical direction of the correction is determined by EBC1: matter contributes $+\Omega_m/x^3$ to the squared Hubble rate, which dominates at small $x$ and drives a *faster* early expansion. The decelerating term $-\Omega_m/(2x^2)$ in EBC2 is smaller in magnitude. The net result is that matter *shortens* the cycle:

$$\frac{T_\mathrm{mat}}{T_\mathrm{free}} \approx 0.87\text{--}0.92$$

i.e., a reduction of approximately **8–13%**. The updated cycle duration is $T \approx \mathbf{190\text{--}320\,\mathrm{Gyr}}$ and the cycle position $f \approx 4\text{--}7\%$. The expansion maximum $x_*$ is unaffected by matter (negligible at $x \sim x_*$). Near the bounce, radiation ($\rho_r \propto a^{-4}$) contributes $\lesssim 0.01$ Gyr to the half-cycle time budget.
...
The dimensionless EBC age follows from integrating EBC1 directly:

$$H_0 \cdot t_0^{(\mathrm{EBC})} = \int_0^1 \frac{dx}{x\,\sqrt{\Omega_m/x^3 + \Omega_r/x^4 + \Omega_{DE}(1/x - x/x_*^2)}} \approx 0.893 \quad(x_0 = 0.06\text{–}0.15,\ \Omega_m = 0.315)$$

compared to the $\Lambda$CDM value $H_0 \cdot t_0 \approx 0.951$. The physical age $t_0 = (H_0 \cdot t_0)/H_0$ depends strongly on the assumed $H_0$. At the $\Lambda$CDM reference value $H_0 = 67.4\,\mathrm{km/s/Mpc}$ one obtains $t_0^{(\mathrm{EBC})} \approx 12.95\,\mathrm{Gyr}$, approximately 6% below the $\Lambda$CDM-inferred 13.8 Gyr. However, a self-consistent EBC fit to DESI Y1 BAO with $r_d$ computed from $(\Omega_m, h)$ via Aubourg et al. (2015) yields $H_0 \approx 58.5\,\mathrm{km/s/Mpc}$, and consequently $t_0^{(\mathrm{EBC})} \approx 15.3\,\mathrm{Gyr}$ — comfortably above the oldest-stellar-population lower limits (Section 8, Item 3c). The 6% deficit at fixed $H_0$ therefore reverses sign when $H_0$ is determined self-consistently from BAO: the age constraint is not a tension but a consistency check that the model satisfies. Crucially, the analytic prediction $w_0 = -2/3$ (Section 2.5) is independent of the numerical integration and is unaffected by these corrections.
...
The currently observed accelerated expansion ($q_0 \approx -0.55$) is fully consistent with the EBC model: we are in the early rising phase of a cycle (estimated 4–7% from numerical integration with physical parameters; Section 3.2), where the elastic restoring term drives positive acceleration. The acceleration will end when the expansion maximum $a_*$ is reached, several tens of billions of years in the future.
...
At the background level, EBC does not resolve the $H_0$ discrepancy. As a dynamical dark-energy model with $w_0 > -1$ that leaves pre-recombination physics essentially unchanged (the elastic term contributes less than $0.0002\%$ of $H^2$ at $z \gtrsim 1100$; Section 8 Item 3c), EBC cannot modify the sound horizon $r_d$ and shares the "late-time-only" limitation of most quintessence-class models. A direct fit of $H_\mathrm{EBC}(z)$ to DESI Y1 BAO with a self-consistent $r_d$ (Section 8 Item 3c) yields $H_0 \approx 58.5\,\mathrm{km/s/Mpc}$, further from the SH0ES distance-ladder value $H_0 \approx 73.0\,\mathrm{km/s/Mpc}$ than $\Lambda$CDM is. The structural reason is transparent: $w_0 > -1$ implies weaker late-time acceleration than $\Lambda$CDM, requiring a smaller $H_0$ to reproduce the same BAO distances. Any late-time-only modification of this kind will fail to close the tension; a complete resolution requires changes to early-universe physics (e.g., early dark energy, additional relativistic species).

At the level of perturbations, the bounce geometry offers a qualitative mechanism that may *contribute* to — but cannot by itself resolve — the tension: slight deformations from internal dynamics (structure formation, gravitational instabilities) accumulated over multiple cycles could lead to spatially varying local expansion rates. The CMB-based $H_0$ measures a large-scale average; local distance-ladder measurements sample a smaller volume that may differ systematically. This interpretation is qualitative only and requires a perturbation-theory treatment beyond the scope of the present background-level framework.
...
3. **Matter and radiation**: Full numerical integration of (EBC1)–(EBC2) with physical parameters ($\Omega_m = 0.315$, $\Omega_r = 9.2 \times 10^{-5}$) has been performed; results are reported in Section 3.2. Matter *shortens* the cycle relative to the matter-free case by 8–13% (ratio $T_\mathrm{mat}/T_\mathrm{free} \approx 0.87$–$0.92$), yielding $T \approx 190$–320 Gyr and $f \approx 4$–7%. The analytic $w_0 = -2/3$ prediction is unaffected.

3a. **One-parameter BAO fit ($H_0$ free, $\Omega_m = 0.315$ fixed).** We fit $H_\mathrm{EBC}(z)$ directly to the DESI Year-1 BAO measurements (12 data points from 7 tracers; [DESI Collaboration 2024]) with $H_0$ as the sole free parameter, $\Omega_m = 0.315$ held at Planck 2018, and $r_d = 147.09$ Mpc at the Planck baseline. Best-fit values span $H_0 = 63.37\,\mathrm{km/s/Mpc}$ ($x_0 = 0.06$) to $H_0 = 63.24\,\mathrm{km/s/Mpc}$ ($x_0 = 0.15$) with $\chi^2/\mathrm{dof} \approx 2.04$–$2.06$ (11 dof). For comparison, $\Lambda$CDM yields $H_0 = 68.17$, $\chi^2/\mathrm{dof} = 1.34$. The resulting EBC age is $t_0 \approx 13.8\,\mathrm{Gyr}$, consistent with independent stellar constraints. EBC exhibits a moderate but non-negligible $\chi^2$ deficit relative to $\Lambda$CDM, traceable primarily to the LRG2 ($z = 0.706$, $D_M$) and LRG3+ELG1 ($z = 0.93$, $D_H$) bins.

3b. **Two-parameter BAO fit ($H_0$, $\Omega_m$ free; $r_d$ fixed).** Extending 3a, we treat $\Omega_m$ as a second free parameter, with $r_d = 147.09$ Mpc still held fixed. Best-fit values are $\Omega_m \approx 0.283$, $H_0 \approx 64.3$–$64.5\,\mathrm{km/s/Mpc}$, $\chi^2/\mathrm{dof} \approx 1.87$–$1.90$ (10 dof). The improvement over 3a is $\Delta\chi^2 \approx -3.7$, or $\Delta\mathrm{AIC} \approx -1.7$ after the AIC penalty for the additional parameter. EBC therefore prefers a lower $\Omega_m$ than Planck 2018 ($0.315$) and lies at the lower edge of the DESI-preferred range ($\approx 0.295$). For $\Lambda$CDM the analogous fit yields $\Omega_m = 0.293$, $H_0 = 69.32$, $\chi^2/\mathrm{dof} = 1.28$. The absolute model-tension $\Delta\chi^2$ between EBC and $\Lambda$CDM at their respective minima is $\approx 5.9$ ($\sim 2.4\sigma$) and is structural rather than parametric: the residuals at LRG2 ($+2.1\sigma$ in $D_M$) and LRG3+ELG1 ($-2.3\sigma$ in $D_H$) persist at the new minimum. A caveat: the fixed $r_d = 147.09$ Mpc is inconsistent with the Aubourg et al. (2015) calibration at the best-fit $(\Omega_m, h)$ (by $\approx 5\%$); fit 3b is therefore an upper bound on the effective model tension rather than a self-consistent constraint.

3c. **Three-parameter BAO fit (self-consistent $r_d$ via Aubourg).** We release $r_d$ from its fixed baseline and compute it self-consistently from $(\Omega_m, h, \Omega_b h^2)$ via the Aubourg et al. (2015) fitting formula [arXiv:1411.1074, Eq. 16]. The baryon density is held at $\Omega_b h^2 = 0.02237$ (Planck 2018 / BBN consensus), so the fit has two effective free parameters. At the drag epoch $z_\mathrm{drag} \approx 1060$ the elastic term contributes less than $0.0002\%$ of $H^2$ (numerically verified), so the Aubourg calibration — derived in $\Lambda$CDM — remains applicable in EBC. Best-fit values for $x_0 = 0.06$: $\Omega_m = 0.282$, $H_0 = 58.5\,\mathrm{km/s/Mpc}$, $r_d = 162.3\,\mathrm{Mpc}$, $\chi^2/\mathrm{dof} = 1.87$ — numerically identical $\chi^2$ to 3b with markedly different $H_0$ and $r_d$. The implied age is $t_0 \approx 15.3\,\mathrm{Gyr}$. The nearly pure $(H_0, r_d)$-degeneration along the BAO observables means that BAO alone cannot disentangle the two; external information (SNe Ia, CMB $\theta_*$) is required. Releasing $\Omega_b h^2$ with a BBN Gaussian prior (standard deviation $0.00015$) leaves all results unchanged, since BAO has no direct sensitivity to $\Omega_b$ separate from $r_d$. The upshot for EBC: the $\Omega_m$ preference and the residual structure are robust conclusions of the BAO fit; $H_0$ cannot be determined from BAO alone without external calibration. When self-consistent Aubourg-$r_d$ is used, the EBC-preferred $H_0 \approx 58.5\,\mathrm{km/s/Mpc}$ lies further from the SH0ES value than $\Lambda$CDM, consistent with the general result that dynamical dark-energy models with $w_0 > -1$ do not alleviate the Hubble tension (Section 4.2).

3d. **Joint BAO + SNe Ia + CMB $\theta_*$ fit** *(deferred).* A joint analysis combining DESI Y1 BAO with Pantheon+ or Union3 supernovae and the CMB acoustic scale $\theta_*$ would break the $(H_0, r_d)$ degeneration exhibited by 3c and provide a sharper test of the EBC background evolution. This is identified as the next development priority.
